...
/// An ERC20 for creating a PVC Token which 
contract PVC is IPVC {
  
    constructor(string memory _name, string memory _symbol, uint8 _decimal, uint256 _totalSupply) {
        name = _name; /// The name of the token
        symbol = _symbol; /// The symbol of the token
        decimal = _decimal; /// The decimal of the token
        totalSupply_ = _totalSupply; /// The total supply of the token
        owner_ = msg.sender; /// Sender address is registered
    }
